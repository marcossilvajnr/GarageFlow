...
public sealed class ExecutionOrderHandlersTests
{
    ...
    [Fact]
    public async Task CreateExecutionOrder_WithEmptyMechanicId_ThrowsDomainException()
    {
        var repo = new FakeExecutionOrderRepository();
        var handler = new CreateExecutionOrderHandler(repo);
        var command = ValidCreateCommand(mechanicId: Guid.Empty);
    ...
    public async Task StartExecutionOrder_WhenPending_ThrowsInvalidExecutionOrderStatusTransitionException()
    {
        ...
        var startHandler = new StartExecutionOrderHandler(repo, soRepo);
        var act = async () => await startHandler.HandleAsync(new StartExecutionOrderCommand(created.Id));
    ...
    public async Task StartExecutionOrder_WhenNotExists_ThrowsEntityNotFoundException()
    {
        ...
        var act = async () => await handler.HandleAsync(new StartExecutionOrderCommand(Guid.NewGuid()));
    ...
    private static async Task<(FakeExecutionOrderRepository, FakeServiceOrderRepository, FakeSeparationOrderRepository, FakeStockRepository, Guid executionOrderId)>
        SeedExecutionInProgressWithSeparationAndStock()
    {
        var repo = new FakeExecutionOrderRepository();
        var soRepo = new FakeServiceOrderRepository();
        ...
        var created = await new CreateExecutionOrderHandler(repo).HandleAsync(new CreateExecutionOrderCommand(serviceOrderId, Guid.NewGuid(), Guid.NewGuid()));
        await new MarkExecutionOrderReadyHandler(repo).HandleAsync(new MarkExecutionOrderReadyCommand(created.Id));
        await new StartExecutionOrderHandler(repo, soRepo).HandleAsync(new StartExecutionOrderCommand(created.Id));
    ...
    {
        var (repo, soRepo, separationRepo, _, executionOrderId) = await SeedExecutionInProgressWithSeparationAndStock();
    ...
    [Fact]
    public async Task CompleteExecutionOrder_DoesNotAlterStockBalance()
    {
        ...
        var stock = (await stockRepo.GetByItemAsync(separation!.Parts[0].PartId, StockItemType.Part))!;
        ...
        // A baixa definitiva ocorre em ConfirmStockistWithdrawal — execução não altera saldo.
        stock.ReservedQuantity.Should().Be(reservedBefore);
        stock.TotalQuantity.Should().Be(totalBefore);
        stock.AvailableQuantity.Should().Be(availableBefore);
    }
